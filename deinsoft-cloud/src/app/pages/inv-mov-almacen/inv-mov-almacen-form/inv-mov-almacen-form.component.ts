...
export class InvMovAlmacenFormComponent implements OnInit {
  ...
  actComprobante: InvMovAlmacen = new InvMovAlmacen();
  ...
  onchangeProduct(event: any, input: any) {
    ...
    this.model.listInvMovAlmacenDet.push(actComprobanteDetalle);
    input.value = '';
  }
  quitarActComprobanteDetalle(e: any): void {
    if (!this.id) {
      this.model.listInvMovAlmacenDet = 
      this.model.listInvMovAlmacenDet.filter(item => item.id != e.id);
    }
    if (this.id) {
  ...
  save() {
    console.log(this.model);
    this.model.numero = "1"
    if (this.model.listInvMovAlmacenDet.length == 0) {
      this.error = []
      this.error.push("Debe agregar productos al movimiento")
      return;
    }
    if (this.model.cnfTipoComprobante.id == 0) {
      this.model.cnfTipoComprobante = null;
    }
    this.invMovAlmacenService.save(this.model).subscribe(m => {
      ...
      this.router.navigate(["/new-mov-almacen"]);
    }, err => {
      if (err.status === 422) {
        this.error = []
  ...
  updateTotals() {
    ...
    this.model.subtotal = 0
    this.model.igv = 0
    this.model.listInvMovAlmacenDet.forEach(element => {
  ...
  removeItem(index:any){
    this.model.listInvMovAlmacenDet.splice(index, 1)
    this.updateTotals()
  }
  addNewCnfMestro() {
