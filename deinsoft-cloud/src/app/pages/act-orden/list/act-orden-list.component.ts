...
@Component({
  selector: 'app-act-orden-list',
  templateUrl: './act-orden-list.component.html',
  providers: [
...
export class ActOrdenListComponent extends CommonReportFormComponent implements OnInit {
  ...
  actOrden: ActOrden = new ActOrden();
  ...
  protected redirect: string = "/act-orden";
  selectedOption: any;
  passwordRepeat: any;
  ...
  getListData() {
    ...
    return this.deps.actOrdenService.getReport(this.model).subscribe(data => {
      this.listData = data;
  ...
  sendApi(item: any) {
    this.deps.utilService.confirmOperation(null).then((result) => {
      if (result) {
        this.deps.actOrdenService.sendApi("act_comprobante", item.id.toString()).subscribe(() => {
  ...
  export() {
    ...
    var extension = "pdf";
    this.deps.actOrdenService.genReportExcel(this.model).subscribe(data => {
      ...
      this.generateAttachment("Lista de compras", blob, extension);
    })
  ...
  anular(item:any) {
    ...
    this.deps.utilService.confirmOperation(null).then((result) => {
      if (result) {
        this.deps.actOrdenService.invalidateActOrden(item.id).subscribe(data => {
          this.deps.utilService.msgOkOperation();
          this.getListData();
        })
      }
  ...
  finalizar(item:any) {
    this.deps.utilService.confirmOperation(null).then((result) => {
      if (result) {
        this.deps.actOrdenService.validateActOrden(item.id).subscribe(data => {
          this.deps.utilService.msgOkOperation();
          this.getListData();
        })
      }
