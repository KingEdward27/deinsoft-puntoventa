...
export class GenericMasterDetailFormComponent extends BaseService  implements OnInit {
  ...
  async save() {
    ...
    this.properties.details.forEach((e:any) => {
      
      if(e.postSaveTrans){
        if(e.type == 'update'){
          let columns = ""
          let condition = ""
          e.postSaveTrans.values.forEach((element:any) => {
            const map = new Map(Object.entries(element));
            columns = columns + element + ",";
          });
          e.postSaveTrans.where.forEach((element2:any) => {
            const map = new Map<string,string>(Object.entries(element2));
            map.forEach((val: string, key: string) => {
              //convMapDetail[key] = val;
          ...
          up.columns = columns
          up.tableName = e.postSaveTrans.tableName
          up.condition = e.postSaveTrans.condition.substring(condition.length-4)
          e.updateParam = up;
          //up.columns = 
          //e.updateParam = 
        }
    ...
    let wa = await this.commonService.createTransactionalAsync(this.properties)
    .catch(err => {
        if (err.status === 422) {
          console.log("error..");
