...
public interface ActContratoRepository extends JpaRepository<ActContrato,Long> {
 ...
 "where upper(p.serie) like %?1% and upper(p.numero) like %?2% and upper(p.observacion) like %?3% and upper(p.flag_estado) like %?4% and upper(p.nro_poste) like %?5% and upper(p.url_map) like %?6% ")

	List<ActContrato> getAllActContrato(long serie,String numero,String observacion,String flagEstado,String nroPoste,String urlMap);
	@Query(value="select p from actContrato p "+
