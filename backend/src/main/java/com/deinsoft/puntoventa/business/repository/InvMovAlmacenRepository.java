...
public interface InvMovAlmacenRepository extends JpaRepository<InvMovAlmacen,Long> {
 ...
 "where upper(p.serie) like %?1% and upper(p.numero) like %?2% and upper(p.numero_ref) like %?3% and upper(p.observacion) like %?4% and upper(p.flag_estado) like %?5% ")

	List<InvMovAlmacen> getAllInvMovAlmacen(long serie,String numero,String numeroRef,String observacion,String flagEstado);
	@Query(value="select p from invMovAlmacen p "+
